...
describe('JSON Node Search', function () {
  ...
  it('should find multiple results', function () {
    const instance = new BufferJsonParser('{"hello": "hello world, is a great world","test": [0,"old world",{"worldgame": true}]}');
    const rootNode = instance.getRootNodeInfo();
    const pattern = /world/;
...
function expectKeyResult(result: TreeSearchMatch, path: string[], index: number, length: number) {
  expect(result.path).toEqual(path);
  expect(result.key).toEqual(index);
  expect(result.value).toBeUndefined();
  expect(result.length).toEqual(length);
}
function expectValueResult(result: TreeSearchMatch, path: string[], index: number, length: number) {
  expect(result.path).toEqual(path);
